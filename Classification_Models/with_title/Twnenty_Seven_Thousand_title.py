...
df = pd.read_csv("../../csv/with_title/27k_amazon_handmade_reviews_balanced.csv")
...
def training_and_testing(model, X_train, y_train, X_test, y_test, filename, filename_Two, original_df):
    ...
    for i, (true, pred) in enumerate(zip(y_test, y_pred)):
        original_text = original_df.iloc[i]["combined_text"]
        if true != pred:
...
svm_training_and_testing, svm_classifier =training_and_testing(svm_classifier,X_train, y_train,X_test,y_test,
                                              "results/svm27k_unsuccessful_results.txt","results/svm27k_successful_results.txt",df)
#printing of training and testing results
with open('training_testing/svm27k_training_and_testing.txt', 'w') as file:
...
nb_training_and_testing, nb_classifier =training_and_testing(nb_classifier,X_train, y_train,X_test,y_test,
                                              "results/nb27k_unsuccessful_results.txt","results/nb27k_successful_results.txt",df)
#printing of training and testing results
with open('training_testing/nb27k_training_and_testing.txt', 'w') as file:
...
xgb_training_and_testing, xgb_classifier =training_and_testing(xgb_classifier,X_train, y_train,X_test,y_test,
                                              "results/xgb27k_unsuccessful_results.txt","results/xgb27k_successful_results.txt",df)
#printing of training and testing results
with open('training_testing/xgb27k_training_and_testing.txt', 'w') as file:
...
joblib.dump(xgb_classifier, 'models/xgb_model_27k.pkl')
...
validate('models/svm_model_27k.pkl', X_unseen, y_unseen,'validated/svm27k_validated.txt')
validate('models/nb_model_27k.pkl', X_unseen, y_unseen,'validated/nb27k_validated.txt')
validate('models/xgb_model_27k.pkl', X_unseen, y_unseen,'validated/xgb27k_validated.txt')
